    ...
    public function indexAction(Request $request)
    {
        ...
        if ($request->isMethod('POST'))
        {
            $form = $this->createFormBuilder($search)
                    ...
                    ->add('Product', ChoiceType::class, array('choices'  => $this->buildChoices3()))
            ...
            if($productname != 0 )
            {
                ...
                $form = $this->createFormBuilder($search)
                    ->add('Manufacturer', ChoiceType::class, array('choices'  => $this->buildChoices()))
                    ->add('Product', ChoiceType::class, array('choices'  => $this->buildChoices2($task)))
                    ->add('name',TextType::class,array('required' => false))
                    ->add('save', SubmitType::class, array('label' => 'Search'))
                    ->getForm();
            }
        ...
        else
        {
            $form = $this->createFormBuilder($search)
                ->add('Manufacturer', ChoiceType::class, array('choices'  => $this->buildChoices()))
                ->add('Product', ChoiceType::class, array('choices'  => $this->buildChoices1()))
                ->add('name',TextType::class,array('required' => false))
                ->add('save', SubmitType::class, array('label' => 'Search'))
                ->getForm();
            $task = 'yes';
